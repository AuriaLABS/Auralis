...
fn run_once(shape: Shape, tile: usize, iters: usize, candidate_first: bool) -> (Duration, Duration) {
    ...
    matmul_reference_into(&a, shape.rows, shape.inner, &b, shape.cols, &mut reference);
    matmul_blocked_cols_into(&a, shape.rows, shape.inner, &b, shape.cols, &mut candidate, tile);
...
fn main() {
    ...
    assert!(repeats > 0 && iters > 0 && tile > 0);
    let shapes = [
    ...
    for shape in shapes {
        ...
        println!(
            ...
            micros(reference_median), micros(candidate_median), ratio,
        );
    }
}
