...
| architecture config schema | `ARCHITECTURE_CONFIG_SCHEMA_VERSION = 3` |
...
`auralis_run_config=1` migra explícitamente a schema 2 con `grad_clip_enabled=true`; schemas de run config distintos de 1/2 se rechazan. Un manifiesto v3 se rechaza. `auralis_architecture=1` migra a LayerNorm + learned-absolute; schema 2 conserva normalización y migra a learned-absolute; schemas distintos de 1/2/3 se rechazan (fail-closed).
