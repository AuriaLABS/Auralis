...
`cargo run --release --bin auralis_gqa_mqa_bench` adds decode and memory curves for MHA, GQA and MQA at contexts 8, 16 and 32. Hosted-runner timing is descriptive. Correctness, finite gradients, compact parameter counts and monotonic/reduced cache memory are hard gates; a quality or latency advantage does not auto-promote a variant.
